                ...
                <div className="grid grid-cols-5 gap-2">
                  {questions.map((_, idx) => (
                    <button
                      ...
                      className={`w-full aspect-square rounded-xl flex items-center justify-center text-sm font-bold transition-all ${currentQuestionIdx === idx ? 'bg-blue-600 text-white shadow-md' :
                ...
                {isSubmitted && (
                  <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 mt-4 text-center">
                    ...
                    {examInfo.resultsPublished ? (
                      ...
                      <p className="text-emerald-600 font-medium italic mt-2 text-sm opacity-80">Assessment finalized. Your results will be available once the coordinator publishes them.</p>
                    )}
                  </div>
                )}
                {!isSubmitted && (
              ...
              <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="space-y-4 pt-4">
                  {activeQuestion.options?.map((opt: string, idx: number) => {
                    ...
                    const showCorrect = isSubmitted && examInfo.resultsPublished && activeQuestion.correctOption === idx;
                    const showWrong = isSubmitted && examInfo.resultsPublished && isSelected && activeQuestion.correctOption !== idx;

                    let stateClass = "bg-white border-slate-200 hover:border-blue-300 hover:shadow-md";
                    if (isSelected && (!isSubmitted || !examInfo.resultsPublished)) stateClass = "bg-blue-50 border-blue-500 ring-1 ring-blue-500 shadow-sm";
                    ...
                    return (
                      ...
                      >
                        <div className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${isSelected && (!isSubmitted || !examInfo.resultsPublished) ? 'bg-blue-600 text-white' :
