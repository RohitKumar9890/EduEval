...
export default function CalendarView({ exams }) {
    ...
    <div className="bg-white rounded-lg">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {[...Array(startingDayOfWeek)].map((_, index) => (
          <div key={`empty-${index}`} className="h-16 bg-gray-50 rounded"></div>
        ))}
        ...
        {[...Array(daysInMonth)].map((_, index) => {
          ...
          return (
            ...
            >
              <div className={`font-semibold ${isTodayDate ? 'text-blue-600' : 'text-gray-700'}`}>
                {day}
              </div>
              {dayExams.length > 0 && (
                <div className="mt-1">
                  {dayExams.slice(0, 2).map((exam, idx) => (
                    <div
                      key={idx}
                      className="text-xs bg-green-100 text-green-800 px-1 rounded truncate mb-1"
                      title={exam.title}
                    >
                      {exam.title}
                    </div>
