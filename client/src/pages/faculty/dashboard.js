...
export default function FacultyDashboard() {
  ...
  // Mock data for assessments needing evaluation
  const assessmentsToGrade = [
    { 
      title: 'Midterm Coding Project: Sorting Algorithms', 
      due: 'Oct 15, 2023', 
      sub: '48/52', 
      icon: 'terminal', 
    ...
    { 
      title: 'Quiz 4: Control Flow & Loops', 
      due: 'Oct 20, 2023', 
      sub: '52/52', 
      icon: 'description', 
    ...
    { 
      title: 'Weekly Lab: Recursion Basics', 
      due: 'Oct 22, 2023', 
      sub: '30/52', 
      icon: 'code_blocks', 
  ...
  // Mock student inquiries
  const inquiries = [
    { 
      name: 'Mark Thompson', 
      time: '10m ago', 
      text: '"Having trouble understanding the edge cases for the sorting midterm..."', 
      avatar: 'https://i.pravatar.cc/150?img=15' 
    },
    { 
      name: 'Sarah Chen', 
      time: '2h ago', 
      text: '"Is there a late submission penalty for the Quiz 4 if the system was down?"', 
      avatar: 'https://i.pravatar.cc/150?img=20' 
    },
    { 
      name: 'James Wilson', 
      time: '4h ago', 
      text: '"Can we use external libraries for the final recursion project?"', 
        ...
        <div className="flex-1 space-y-8">
          <div className="flex flex-wrap justify-between items-end gap-6">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-base font-medium">
                Manage your course assessments and student evaluations.
              </p>
            </div>
            <button 
              onClick={() => router.push('/faculty/exams')}
              className="bg-primary hover:bg-primary/90 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-primary/20 transition-all active:scale-95"
            >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {assessmentsToGrade.map((item, i) => (
                <div 
                  key={i} 
                  className="flex flex-col sm:flex-row items-start sm:items-center gap-6 px-8 py-6 justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      <div className="flex -space-x-3">
                        {[1, 2, 3].map(a => (
                          <div 
                            ...
                            style={{ backgroundImage: `url('https://i.pravatar.cc/150?img=${a + 10}')` }} 
                          />
                        ))}
                        <div className="size-9 rounded-full border-2 border-white dark:border-slate-900 bg-slate-800 flex items-center justify-center text-[10px] text-white font-black">
                          +12
                        </div>
                      </div>
                    )}
                    <button className="flex min-w-[130px] items-center justify-center rounded-xl h-11 px-5 bg-primary text-white text-xs font-black uppercase tracking-widest shadow-lg shadow-primary/20 transition-all hover:opacity-95 active:scale-95 group">
                      ...
                      <span className="ml-3 bg-white text-primary text-[10px] size-5 rounded-full flex items-center justify-center transition-transform group-hover:scale-110">
                        {item.count}
                      </span>
                    </button>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm transition-colors">
            ...
            <div className="space-y-5">
              {inquiries.map((q, i) => (
                <div 
                  key={i} 
                  className="group p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 transition-all hover:border-primary/40 cursor-pointer shadow-sm hover:shadow-md"
                >
                  <div className="flex items-start gap-3">
                    <div 
                      className="size-9 rounded-full bg-cover bg-center border-2 border-white dark:border-slate-800 shrink-0" 
                      style={{ backgroundImage: `url('${q.avatar}')` }}
                    ></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <p className="text-xs font-black text-slate-900 dark:text-white truncate uppercase tracking-widest">
                          {q.name}
                        </p>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{q.time}</p>
                      </div>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium italic leading-relaxed line-clamp-2">
                        {q.text}
                      </p>
            ...
            <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex flex-wrap gap-3">
                {['Syllabus', 'Class Roster', 'Office Hours'].map(link => (
                  <button 
                    key={link} 
                    className="px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-800 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-sm"
                  >
                    {link}
                  </button>
                ))}
